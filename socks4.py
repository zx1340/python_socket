...
def changeFilter(command):
    ...
    if command[1] == 'ban':
        if command[2] in Filter['BanIP']:
            return '[I]Ip already banned'
    ...
    if command[1] == 'len':
        ...
        if command[2] not in Filter['Size']:
...
class MSock4(SOCKSv4):
    ...
    def handlePackage(self,Mode,data):
        ...
        if Filter['Mode'] == 'Send' and Mode == 'Send': return False
        if Filter['Mode'] == 'Recv' and Mode == 'Recv': return False
        if csinfo[0] in Filter['BanIP'] or csinfo[2] in Filter['BanIP']: return False
    ...
    def runCommand(self,cmd):
        ...
        if command[0] == 'repeat':
            ...
            if len(command) != 2 and len(command) != 3:
                return '[E]Using: filter [addport|removeport|sendonly|recvonly] + [port]'
            else:
